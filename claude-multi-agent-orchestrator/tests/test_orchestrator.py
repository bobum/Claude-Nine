...
class TestPostCompletionPhases:
    """Test post-completion phases (push, merge, PR creation)."""
...
class TestPRCreation:
    ...
    def test_create_pull_request_success(self, temp_git_repo):
        ...
        try:
            with patch('orchestrator.requests.get') as mock_get:
                ...
                with patch('orchestrator.subprocess.run') as mock_run:
                    ...
                    # Check gh command was called with correct args
    ...
    def test_create_pull_request_single_feature(self, temp_git_repo):
        """Test PR title for single feature."""
        (Path(temp_git_repo) / ".agent-workspace").mkdir()
        tasks_path = Path(temp_git_repo) / "tasks.yaml"
        tasks_path.write_text("features: []")
        ...
        try:
            with patch('orchestrator.requests.get') as mock_get:
                ...
                with patch('orchestrator.subprocess.run') as mock_run:
                    ...
                    # Check title contains feature name
                    call_args = mock_run.call_args[0][0]
                    title_idx = call_args.index("--title") + 1
                    assert "feature/login" in call_args[title_idx]
        finally:
            os.chdir(old_cwd)

    def test_create_pull_request_gh_not_found(self, temp_git_repo):
    ...
    def test_create_pull_request_gh_error(self, temp_git_repo):
